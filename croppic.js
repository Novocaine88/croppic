...
(function(window, document) {
	...
	Croppic.prototype = {
			...
			that.img.on('mousedown touchstart', function(e) {
				...
				that.img.css('z-index', 1000).on('mousemove touchmove', function(e) {
					...
					if (that.objH < that.imgH) {
						if (parseInt(that.img.css('top'), 10) > 0) {
							that.img.css('top', 0);
							if (that.options.imgEyecandy) {
						...
						if (parseInt(that.img.css('top'), 10) < maxTop) {
					...
					if (that.objW < that.imgW) {
						if (parseInt(that.img.css('left'), 10) > 0 ) {
							that.img.css('left', 0);
							if (that.options.imgEyecandy) {
						...
						if (parseInt( that.img.css('left'), 10) < maxLeft) {
		...
		zoom: function(x) {
			...
			var newTop = parseInt( that.img.css('top'), 10) - x / 2;
			var newLeft = parseInt( that.img.css('left'), 10) - x / 2;
		...
		crop: function() {
			...
			var cropData = {
					...
					imgW: that.imgW,
					imgH: that.imgH,
					imgY1: Math.abs(parseInt(that.img.css('top'), 10)),
					imgX1: Math.abs(parseInt(that.img.css('left'), 10)),
					cropH: that.objH,
					cropW: that.objW,
			...
			$.ajax({
				...
				type: 'POST'
					...
					if (that.options.onAfterImgCrop) {
						that.options.onAfterImgCrop.call(that, response.status);
					}
